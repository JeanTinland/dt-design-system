...
const DateItem = ({ date, month, year, value, onChange, minDate, maxDate }) => {
  ...
  const inMonth =
    month &&
    year &&
    Utils.isSameMonth(_date, new Date([Utils.zeroPad(year, 2), Utils.zeroPad(month, 2), '01'].join('-')))

  const isDisabled = (minDate && _date < minDate) || (maxDate && _date > maxDate)
  ...
  return (
    <button className={classes} onClick={updateDateSelection} tabIndex={isDisabled ? -1 : 0}>
      {_date.getDate()}
    </button>
  )
}
